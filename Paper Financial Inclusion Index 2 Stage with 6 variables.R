...
Paperdataset<-
  Paperdataset %>%
  mutate(Borrowers = coalesce(i_borrowe rs_A1_pop, i_loan_acc_A1_pop))

Analysis<-
  Paperdataset%>%
  filter(!is.na(Borrowers) &!is.na(Depositors) &!is.na(i_ATMs_km2) &!is.na(i_branches_km2_A1) &!is.na(i_deposits_A1_GDP) &!is.na(i_loans_A1_GDP))
Analysis<-
  Analysis%>%
  filter(year>2008)
...
Analysisset<-Analysis[,c("economy","year","group_region","group_income","group_IMF","denom_gdp",
                         ...
                         "i_ATMs_km2","i_branches_km2_A1","Depositors","Borrowers","i_deposits_A1_GDP","i_loans_A1_GDP")]
...
for (i in 2009:2017){
  Analysisset$Depositsnorm1<-Analysisset$i_deposits_A1_GDP/max(Analysisset$i_deposits_A1_GDP)
}

for (i in 2009:2017){
  Analysisset$Loansnorm1<-Analysisset$i_loans_A1_GDP/max(Analysisset$i_loans_A1_GDP)
}
...
pca2<-Analysisset[,c("Depositorsnorm1","Borrowersnorm1","Depositsnorm1","Loansnorm1")]
...
Analysisset$Usage<-0.4668952^2*Analysisset$Depositorsnorm1+0.4874219^2*Analysisset$Borrowersnorm1+0.4848224^2*Analysisset$Depositsnorm1+ 0.5562158^2*Analysisset$Loansnorm1
...
write.csv(Results,"TwostagePCAresultusing6indicators.csv")
